...
- `db.py` — schema creation + `SCHEMA_VERSION` migration. Tables: `documents` (with `tags` column), `chunks` (with `doc_path` + `fts_path` columns), `vec_chunks` (vec0 virtual table), `fts_chunks` (FTS5 with 3 columns: doc_path/heading/text, weighted BM25, porter stemming, trigger-based sync using `fts_path`). WAL mode + foreign keys enabled. `fts_path()` returns last 2 path components to avoid IDF collapse from common prefixes. v7→v8 adds `fts_path` + rebuilds FTS with truncated paths; v6→v7 adds `doc_path` to chunks + FTS with field weights; v5→v6 rebuilds FTS with porter tokenizer; v4→v5 rebuilds FTS with triggers; v3→v4 uses ALTER TABLE; older versions drop-and-recreate.
...
- `filters.py` — inline filter syntax (`file:`, `type:`, `tag:`, `dt>`, `dt<`, `+"kw"`, `-"kw"`) parsed from query string, applied post-search. `get_tag_chunk_count()` computes tagged doc chunk count for over-fetch when `tag:` filter is active.
...
**Search**: query → parse_filters → [HyDE best-of-two: embed both raw query + HyDE passage in one batch, run two vec queries, keep set with better top-1 similarity] → [expand] → vec0 MATCH + FTS5 MATCH (original + expansion queries) → multi-list weighted RRF (primary 2.0, expansions 1.0) → tag over-fetch if `tag:` active → apply_filters → display. `fused` count captured pre-filter; `after_filters` added when filters active.
...
**Ask**: question → parse_filters → BM25 probe (LIMIT 20, deduplicate by document, shortcut if top-doc norm >= 0.85 with gap >= 0.15 vs second-doc) → if shortcut: FTS only; else: [HyDE best-of-two] → [expand] → hybrid search + tag over-fetch → multi-list weighted RRF → rerank (cross-encoder or LLM) → top rerank_top_k → confidence threshold → LLM generates answer.
...
- **BM25 shortcut in ask** — probes top 20 FTS results, deduplicates by document (keeps best norm per doc), compares top-doc vs second-doc; if top norm >= 0.85 with gap >= 0.15, skips HyDE/expansion/embedding/vector/rerank and uses FTS results directly. Document-level dedup prevents same-doc chunks from suppressing the gap.
- **Tag over-fetch** — when `tag:` filter is active, `retrieve_k` is increased to cover all chunks from tagged documents (`get_tag_chunk_count() + top_k * 3`), ensuring tagged docs aren't missed by the fixed top-k retrieval window
- **Schema versioning** — `SCHEMA_VERSION` in `meta` table; v7→v8 adds `fts_path` to chunks + rebuilds FTS with truncated paths (manual rebuild, not FTS5 built-in, because `content='chunks'` maps to `doc_path`); v6→v7 adds `doc_path` to chunks + FTS with field weights; v5→v6 rebuilds FTS with porter tokenizer; v4→v5 rebuilds FTS with triggers; v3→v4 uses non-destructive ALTER TABLE; older upgrades drop and recreate all tables
