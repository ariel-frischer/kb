...
- `embed.py` — embedding dispatcher: `embed_batch()` routes to `local_embed_batch()` (SentenceTransformer, default `Snowflake/snowflake-arctic-embed-m-v1.5`, 768d, Matryoshka truncation) or OpenAI API based on `cfg.embed_method`. Local model lazy-loaded and cached (`_embed_model_cache`), GPU if available (CUDA > MPS > CPU). `is_query` param controls arctic-embed prefix (`prompt_name="query"` for queries, no prefix for documents). Also provides `serialize_f32()` / `deserialize_f32()` for sqlite-vec binary format
...
- **Local embedding** — `embed_method = "local"` uses `local_embed_model` (default `Snowflake/snowflake-arctic-embed-m-v1.5`, 109M params, 768d) via sentence-transformers. Lazy-loaded and cached (`_embed_model_cache`), GPU if available. `is_query` flag in `embed_batch()` controls arctic-embed query prefix (`prompt_name="query"`). Supports Matryoshka truncation if `embed_dims < 768`. Switching `embed_method` after indexing requires `kb index --reindex` (dimension mismatch). Ingest creates `OpenAI()` client only when `embed_method != "local"`, enabling fully offline indexing
